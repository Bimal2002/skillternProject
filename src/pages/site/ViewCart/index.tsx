...
//                           onChange={(e) => dispatch(setSelectedCoupon(e.target.value))}
//                         />
//                         <ButtonCmp className='btn btn-sm'>Apply</ButtonCmp>
//                       </Space.Compact>
//                     </div>
...
const ViewCart = () => {
  ...
  const { data: totalPriceData, refetch: refetchTotalPrice } = priceQuery({
    courseIds: courseIds.join(','),
    couponCode: selectedCoupon || undefined
  });
  ...
  useEffect(() => {
    refetchTotalPrice();
  }, [selectedCoupon, refetchTotalPrice]);
  ...
  const checkoutHandler = () => {
    ...
    if (isAuth) {
      ...
      navigate('/checkout', {
        state: {
          totalPrice: totalPriceData?.totalPrice ?? totalPrice
          ...
          <div className='view-cart__content '>
            <Row className='row-wrap'>
              ...
              <Col md={6} className='col view-cart__right'>
                <div className='view-cart__summary'>
                  <h4 className='view-cart__summary-title'>Total: </h4>
                  <h3 className='view-cart__summary-price'>${totalPriceData?.totalPrice ?? totalPrice}</h3>
                  <div onClick={checkoutHandler}>
                    <div className='view-cart__summary-btn btn btn-md'>Checkout</div>
                  </div>
                  <Divider />
